...
@foreach(App\Book_pic::all() as $pic)

<a href="#">
<div class="row" id="ad-row">
    <div class="col-md-6">
            <img src="{{asset('uploads/images/').'/'.$pic->name.'.'.$pic->extension }}" class="img-responsive" alt="" width="300px" height="300px">
    </div>
    <div class="col-md-6">
    <?php $predefinedAddressLength = 25; ?>
        <h3>{{ $book->name }}</h3>
        <h4>{{ $book->branch['name'] }}</h4>
        <h4>Seller : {{ $book->user()->getResults()['first_name']}} {{ $book->user()->getResults()['last_name'] }}</h4>
